# hoshiyomi（星詠み）— Claude Code マスタールール

このフォルダは **hoshiyomi**（AI × スピリチュアルのスモールビジネス）を Claude Code で運営するためのベースです。
Boris Cherny の Claude Code 運用 Tips を「ファイルに落とせる部分」と「習慣として守る部分」に分け、前者は本ディレクトリの設定ファイルに、後者は本 CLAUDE.md に集約しています。

> プロダクト仕様は `SPEC.md` を参照。
...
**ブランド**: hoshiyomi（星詠み）
**カテゴリ**: AI × スピリチュアル（四柱推命・西洋占星術・タロット）
**目標**: Phase 1 で月5万円 → Phase 3 で月20万円+
**運営者の制約**: 顔出し NG / テキスト+画像で SNS 運用
**運営者の強み**: AI を使った占い・四柱推命アプリの開発経験あり

### 事業設計の優先順位
1. **自動化最優先** — 寝ている間に売れる仕組みを最初に作る
2. **ストック資産を積む** — PDF テンプレ・AI プロンプト・SNS 投稿テンプレを資産化
3. **SNS は LP の代わり** — X / Threads / Instagram / note / LINE が販売チャネル
4. **コアファン化を最優先 KPI に** — 短期売上より「コアファンの数」を見る

### スピリチュアル系コンテンツの執筆ガイドライン（重要）
- 「絶対に〇〇になる」「必ず〇〇が起こる」など断定的・誇大表現は **禁止**（景表法対応）
- 「〜という傾向があります」「〜と言われています」など余白を残す表現を使う
- 医療・治療・健康効果を謳う表現は **禁止**（薬機法対応）
- スタンスは「**自己理解の手助け / 毎日のお守り体験**」を貫く
- 鑑定 AI のプロンプトも上記ガイドに沿って設計する
